...
function setup(failure){
 ...
 const ctx={FieldValue:firestore.FieldValue,admin:{firestore},logger:{info:()=>{}}};vm.createContext(ctx);vm.runInContext(fn,ctx);
 return {data,run:()=>ctx.seedProductionCompany('plant',{companyName:'QA',adminPin:'1234'})};
}
(async()=>{
 const successful=setup();await successful.run();const baseline=JSON.stringify([...successful.data]);await successful.run();assert.equal(JSON.stringify([...successful.data]),baseline);assert.equal(successful.data.size,8);assert(successful.data.has('companies/plant/roles/Admin'));assert(successful.data.has('companies/plant/users/1000'));assert(![...successful.data.keys()].some(k=>/\/areas\/|\/stations\//.test(k)));
