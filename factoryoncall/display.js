...
(async function () {
  ...
  function displayCallStationName(call = {}) {
    return String(call.station || call.stationName || "").trim();
  }

  function isDisplayableSoundCall(call = {}) {
    const station = displayCallStationName(call);
    if (!station || station.toLowerCase() === "unknown station" || station.toLowerCase() === "unknownstation") return false;
    return true;
  }
  ...
  function handleCallSoundSnapshot(snapshot) {
    ...
    snapshot.docChanges().forEach(change => {
      ...
      if (!isDisplayableSoundCall(call) || isEmergencyCallRecord(call)) {
        rememberCallSoundState(call);
        return;
      }

      const status = displayCallStatus(call);
